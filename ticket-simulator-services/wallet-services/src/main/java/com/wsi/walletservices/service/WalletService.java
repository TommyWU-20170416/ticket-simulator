...
public class WalletService {
     ...
     */
    @Transactional
    public Wallet payPessimisticWrite(Integer walletId, Integer amount) {
    ...
    public Wallet payOptimisticWrite(Integer walletId, Integer amount) {
        ...
        for (int i = 0; i < MAX_RETRY; i++) {
            ...
            try {
                return attempPayOptimisticWrite(walletId, amount);
    ...
    public Wallet attempPayOptimisticWrite(Integer walletId, Integer amount) {
        Wallet wallet = walletRepository.findById(walletId)
                .orElseThrow(() -> new WalletNotFoundException("錢包不存在"));
        if (wallet.getWalletAmount().compareTo(amount) < 0) {
            throw new RuntimeException("餘額不足");
        }
        wallet.setWalletAmount(wallet.getWalletAmount() - amount);
